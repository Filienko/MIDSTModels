...
def conditional_sampling_by_group_size(
    ...
    while curr_index < len(ys):
        ...
        curr_ys = torch.tensor(
            np.array(ys[curr_index:end_index]).reshape(-1, 1), requires_grad=False
        )
        curr_model_kwargs = {}
        curr_model_kwargs["y"] = curr_ys
        curr_sample, _ = diffusion.conditional_sample(
            ys=curr_ys, model_kwargs=curr_model_kwargs, cond_fn=cond_fn
        )
    ...
    num_numerical_features_sample = num_numerical_features + int(
        dataset.is_regression and not is_y_cond
    )
    ...
    if num_numerical_features != 0:
        ...
        if len(label_encoders) > 0:
            ...
            decoded_x_cat = []
            for col in range(X_cat.shape[1]):
                decoded_x_cat.append(
                    label_encoders[col].inverse_transform(X_cat[:, col])
                )
            X_cat = np.column_stack(decoded_x_cat)
...
def conditional_sampling(
    ...
    all_rows = []
    all_clusters = []
    _, empirical_class_dist = torch.unique(torch.from_numpy(labels), return_counts=True)

    while len(all_rows) * sample_batch_size < num_samples:
        classes = torch.randint(
            low=0,
            high=len(empirical_class_dist),
    ...
    for i in range(len(arr)):
        ...
        with torch.no_grad():
            pred = classifier(
                curr_sample, timesteps=torch.zeros(curr_sample.shape[0]).to(device)
            )
            pred = pred.argmax()
...
def match_rows(A, B):
    ...
    while A.shape[0] > 0:
        ...
        nearest_neighbors_indices, _ = match_tables(
            A, B, n_clusters=25, unique_matching=False, batch_size=100
        )
...
def handle_multi_parent(
    ...
    synthetic_child_dfs = [
        (synthetic_tables[(parent, child)]["df"].copy(), parent) for parent in parents
    ]
